...
params: 
  ...
  folder_loc: "2022.01"
  start_year: 2018
  end_date_label: "February 28, 2022"
...
phn_flag_month <- bcehs2 %>%
  group_by(dateofevent_floor, year, month_lab, phn_flag_update) %>%
  dplyr::summarise(total =n()) %>%
  mutate(percent=round(total/sum(total)*100, digits=3))
...
```{r plot_phn_month, fig.width=14}
phn_flag_month %>% filter(dateofevent_floor >= ymd("2018-01-01") & phn_flag_update == "Y") %>%
   ggplot(aes(dateofevent_floor, percent, fill=phn_flag_update)) +
  #geom_line()  +
...
od_event <- bcehs2 %>%  #filter(year >= "2018") %>%
   group_by(dateofevent_floor, FN_FLAG) %>% 
...
od_event %>% 
  filter(FN_FLAG ==0) %>% 
  ggplot(aes(x=dateofevent_floor, y=total,group=FN_FLAG, fill=FN_FLAG)) + geom_bar(stat="identity", position= "dodge")+
...
ggsave(file = "od_event, device.png", path = "//fnha.local/groups/Health Surveillance/HS Staff - Confidential/Opioid Overdose Crisis/SORT/sort_analyst_files/January", width = 12) 
...
flag_dates <- seq(min(od_prop_complete$dateofevent_floor), max(od_prop_complete$dateofevent_floor), by = '2 month')

od_prop_complete <- od_prop_complete %>% 
  mutate(text_labels = ifelse(dateofevent_floor %in% flag_dates | dateofevent_floor == max(od_prop_complete$dateofevent_floor), percent, NA), 
         text_pos_x = ifelse(dateofevent_floor %in% flag_dates, 0, 
                             ifelse(dateofevent_floor == max(od_prop_complete$dateofevent_floor), 1, 
                                    NA)))
...
od_prop_complete %>% filter(FN_FLAG == "1") %>%
  ggplot(aes(x=dateofevent_floor, y=proportion, group=FN_FLAG)) +
...
od_prop_join<-od_prop_complete %>% select(dateofevent_floor, FN_FLAG, proportion)
```


```{r}
od_event_phn <- phn_flag_month %>% filter(phn_flag_update == "Y") %>% 
  filter(dateofevent_floor >= ymd("2020-01-01") ) %>%
  select(dateofevent_floor, phn_flag_update, percent) %>% 
  left_join(od_event, by="dateofevent_floor") %>% 
  rename(percent_phn = percent) %>% 
  left_join(od_prop_join, by=c("dateofevent_floor", "FN_FLAG")) %>% 
  filter(FN_FLAG ==1) %>% 
...
od_event_phn %>% 
  ggplot(aes(x=dateofevent_floor, y=total,group=FN_FLAG, fill=FN_FLAG)) + 
  geom_bar(stat="identity", position= "dodge")+
  # geom_line(aes(x=dateofevent_floor, y=expected_prop_base, color="Estimated ODs"), linetype="dashed", group=1)+
 # geom_line(aes(x=year_mm, y=expected_ratio_base), linetype="dotted", color="magenta")+
  ...
  scale_x_date(labels = format_dates, date_breaks = "1 months", expand = c(0.01, 0.01))+
  ...
  labs(title = "Number of Drug Poisonig (& Expected) among First Nations people by Month", 
       subtitle = date_range_labels, 
       x="Month", y="Total Drug Poisonings")

# ggsave(here(params$folder_loc, "fn_events_expected_events2.jpeg"), width=12)
ggsave(file = "fn_events_expected_events2.jpeg", path = "//fnha.local/groups/Health Surveillance/HS Staff - Confidential/Opioid Overdose Crisis/SORT/sort_analyst_files/April" , width = 12)
...
pop <- pop <- read_csv(file="//fnha.local/groups/Health Surveillance/HS Staff - Confidential/Opioid Overdose Crisis/Opioid Quarterly Reports/R_readin_2020.08.05_Population_File.csv")
...
by_month <- bcehs2 %>% #filter(year >= "2018") %>%
  dplyr::group_by(year_mm,FN_FLAG, year) %>%
  dplyr::summarise(total_od = n()) %>%
...
od_event_sex <- bcehs2 %>%  
  filter(PAT_GENDER != "NA") %>% filter(year >= "2020") %>%
   group_by(dateofevent_floor, FN_FLAG, PAT_GENDER) %>% 
  summarise(total= n())

sex_labels <- c("0" = "Other Residents", "1" ="First Nations")

flag_dates <- seq(min(od_event_sex$dateofevent_floor), max(od_event_sex$dateofevent_floor), by = '2 month')
od_event_sex <- od_event_sex %>% 
  mutate(text_labels = ifelse(dateofevent_floor %in% flag_dates | dateofevent_floor == max(od_event_sex$dateofevent_floor), total, NA))
...
od_event_sex %>% filter(FN_FLAG ==1) %>% 
  ggplot(aes(x=dateofevent_floor, y=total,group=PAT_GENDER, color=PAT_GENDER)) + 
  scale_x_date(labels = format_dates , date_breaks = "1 months", expand=c(0.015,0.015))+ 
  ...
  geom_point(aes(color=PAT_GENDER))+ 
    standards +
  geom_text_repel(aes(label=total), size=8)+
...
ggsave(file = "od_events_sex_fn.jpeg", path = "//fnha.local/groups/Health Surveillance/HS Staff - Confidential/Opioid Overdose Crisis/SORT/sort_analyst_files/April" , width = 12, height = 7)

```

one_yr_roll <- seq(max(bcehs2$DATE_SERVICE), by="-1 month", length = 12)
one_yr_roll



by_sex <- bcehs2 %>% 
  filter(PAT_GENDER != "NA") %>% #filter(year >= "2018") %>%
  filter(FN_FLAG == 1) %>%
  dplyr::group_by(DERIVED_HA, ha_name, FirstNations, PAT_GENDER) %>% 
  dplyr::summarise(total= n()) %>%
  dplyr::mutate(percent = 100*total/sum(total)) %>%
  mutate(ha_name = as.character(ha_name))
# labels_fn <- c( "0" ="Other Residents", "1" ="First Nations")
labels_fn <- c("other_residents"="Other Residents", "first_nations"="First Nations")

# FN_labels <- c("0" = "Other Residents", "1" ="First Nations")

 by_sex %>% 
  filter(PAT_GENDER != "Unknown") %>%
  ggplot(aes(ha_name, total, fill = fct_relevel(PAT_GENDER, "M"))) + 
  geom_bar(stat = "identity", position = "fill") + #make bars and make them side by side 
  scale_fill_manual(values = (c("slateblue2" , "lightgoldenrod1"  )), guide=guide_legend(reverse = T)) + coord_flip() +
  standards +
  facet_wrap(FirstNations ~., labeller=labeller(FirstNations=labels_fn)) +
   # facet_grid(FN_FLAG~., scales ="free", labeller=labeller(FN_FLAG=FN_labels)) +
  geom_text(aes(label=paste0(round(percent, digits = 1), "%"
                            ,  "\n", "n=", total
                             )), 
            position = position_fill(vjust = 0.5), size = 6.5,
           ) + 
  theme(legend.position = "bottom", 
        strip.text.x = element_text(
          size = 14, color = "black", face = "bold")) +
  # scale_x_discrete(labels = c("Interior" , "Fraser",  "Vancouver Coastal" , "Vancouver Island", "Northern"  )) + #add labels
  # scale_y_discrete(labels = c("Other Residents" , "First Nations")) + #add labels

  scale_y_continuous(labels = scales::percent, breaks = seq(0, 1, by = 0.5)) +
  labs(title = "Drug Poisoning Events by Region, 1 Year Rolling", 
       subtitle = paste(format(min(one_yr_roll), "%B %d, %Y"), "-", format(max(one_yr_roll), "%B %d, %Y")),
       x = " ", y="Percentage")

# ggsave("adhoc_sex_region_1_yr_roll_BCEHS.jpeg", path = "L:/Health Surveillance/HS Staff - Confidential/Opioid Overdose Crisis/Opioid Quarterly Reports/2021/September", width =  12.85, height = 7.8)
ggsave("adhoc_sex_region_1_yr_roll_BCEHS.jpeg", path = "//fnha.local/groups/Health Surveillance/HS Staff - Confidential/Opioid Overdose Crisis/SORT/sort_analyst_files/April",  width =  12.85, height = 7.8)




```{r}
by_ha_mm <- bcehs2 %>% filter(DERIVED_HA %in% c(1,2,3,4,5))%>% #filter(year >= "2020") %>%
   group_by(dateofevent_floor,FN_FLAG,FirstNations,DERIVED_HA) %>%
...
flag_dates <- seq(min(by_ha_mm$dateofevent_floor), max(by_ha_mm$dateofevent_floor), by = '2 month')
by_ha_mm <- by_ha_mm %>% 
  mutate(text_labels = ifelse(dateofevent_floor %in% flag_dates | dateofevent_floor == max(by_ha_mm$dateofevent_floor), ha_total_od, NA))
...
###IGNORE THE GRAPH BELOW"
 by_ha_mm %>%  #filter(year(year_mm)>=2020) %>% 
   filter(FN_FLAG==1) %>%
   # filter(dateofevent_floor %in% one_yr_roll) %>%
  ggplot(aes(x=dateofevent_floor, y=ha_total_od, group=ha_name)) + 
   scale_x_date(labels = format_dates, date_breaks = "1 months", expand = c(0.015, 0.015))+ 
...
```{r ha_stack}
 by_ha_mm %>%  #filter(year(year_mm)>=2020) %>% 
   filter(FN_FLAG==1) %>% 
  ggplot(aes(x=dateofevent_floor, y=ha_total_od, fill=ha_name)) + 
  # facet_grid(FN_FLAG~., labeller = labeller(FN_FLAG = FN_labels), scales = "free")+ 
 ...
 ggsave(here(params$folder_loc, "od_events_ha_fn_2020.jpeg"), width=12, height=7)
 # ggsave(file = "od_events_ha_fn_2020_vs4.jpeg", path = "L:/Health Surveillance/HS Staff - Confidential/Opioid Overdose Crisis/SORT/sort_analyst_files/July" , width = 12, height = 7) 

```
#### Make sure to change the fiter for dateofevent_floor####
by_ha_mm %>%  #filter(year(year_mm)>=2020) %>% 
  filter(FN_FLAG==1) %>%
  filter(dateofevent_floor >= "2021-04-01") %>%
  ggplot(aes(x=dateofevent_floor, y=ha_total_od, fill=ha_name)) + 
  # facet_grid(FN_FLAG~., labeller = labeller(FN_FLAG = FN_labels), scales = "free")+ 
  geom_bar(stat = "identity", position = "stack")+ 
  geom_text(aes(label=ha_total_od), position = position_stack(vjust = 0.5), size = 5) +
 ...
 # ggsave(here(params$folder_loc, "od_events_ha_fn_2020.jpeg"), width=12, height=7)
ggsave("od_events_ha_FN.jpeg", path = "//fnha.local/groups/Health Surveillance/HS Staff - Confidential/Opioid Overdose Crisis/SORT/sort_analyst_files/April" , width = 6, height = 6)
...
by_ha_mm %>%  #filter(year(year_mm)>=2020) %>% 
  ...
  geom_text(aes(label=ha_total_od), position = position_stack(vjust = 0.5), size = 5) +
   ...
   # geom_label(data=pan_label_fn, aes(label=covid_label, x=x, y=y+150), fill="white") +
    scale_x_date(labels = format_dates, date_breaks = "1 months", expand = c(0.015, 0.015))+ 
    scale_y_continuous(limits = c(0, 120), breaks = seq(0, 200, by=30)) +
     standards +
   theme(axis.text.x = element_text(angle=45,hjust=1), 
         plot.title = element_text(size = 13.5), 
         legend.position = "bottom", 
         legend.title = element_blank())+ #geom_text(aes(label=text_labels), size=4)+
    labs(title = "Monthly Overdose Events ", 
         # subtitle = paste(params$start_date_label, "-", params$end_date_label),
         x = "Month-Year",  y = "Number of Overdoses")
 
 # ggsave(here(params$folder_loc, "od_events_ha_fn_2020.jpeg"), width=12, height=7)
ggsave("od_events_ha_INTERIOR.jpeg", path = "//fnha.local/groups/Health Surveillance/HS Staff - Confidential/Opioid Overdose Crisis/SORT/sort_analyst_files/April" , width = 6, height = 6)
...
by_ha_mm %>%  #filter(year(year_mm)>=2020) %>% 
 ...
 # ggsave(here(params$folder_loc, "od_events_ha_fn_2020.jpeg"), width=12, height=7)
ggsave("od_events_ha_FRASER.jpeg", path = "//fnha.local/groups/Health Surveillance/HS Staff - Confidential/Opioid Overdose Crisis/SORT/sort_analyst_files/April" , width = 6, height = 6)
```
...
by_ha_mm %>%  #filter(year(year_mm)>=2020) %>% 
  ...
  geom_text(aes(label=ha_total_od), position = position_stack(vjust = 0.5), size = 5) +
   ...
   # geom_label(data=pan_label_fn, aes(label=covid_label, x=x, y=y+150), fill="white") +
    scale_x_date(labels = format_dates, date_breaks = "1 months", expand = c(0.015, 0.015))+ 
    scale_y_continuous(limits = c(0, 130), breaks = seq(0, 200, by=30)) +
     standards +
   theme(axis.text.x = element_text(angle=45,hjust=1), 
         plot.title = element_text(size = 13.5), 
         legend.position = "bottom", 
 ...
 # ggsave(here(params$folder_loc, "od_events_ha_fn_2020.jpeg"), width=12, height=7)
ggsave("od_events_ha_VC.jpeg", path = "//fnha.local/groups/Health Surveillance/HS Staff - Confidential/Opioid Overdose Crisis/SORT/sort_analyst_files/April" , width = 6, height = 6)
```
...
by_ha_mm %>%  #filter(year(year_mm)>=2020) %>% 
 ...
 # ggsave(here(params$folder_loc, "od_events_ha_fn_2020.jpeg"), width=12, height=7)
ggsave("od_events_ha_VI.jpeg", path = "//fnha.local/groups/Health Surveillance/HS Staff - Confidential/Opioid Overdose Crisis/SORT/sort_analyst_files/April" , width = 6, height = 6)
```
...
by_ha_mm %>%  #filter(year(year_mm)>=2020) %>% 
 ...
 # ggsave(here(params$folder_loc, "od_events_ha_fn_2020.jpeg"), width=12, height=7)
ggsave("od_events_ha_Northern.jpeg", path = "//fnha.local/groups/Health Surveillance/HS Staff - Confidential/Opioid Overdose Crisis/SORT/sort_analyst_files/April" , width = 6, height = 6)
```
...
pop21 <- pop %>% filter(year == 2020) %>% mutate(year = 2021)
pop_combo <- bind_rows(pop, pop21) %>% arrange(FN_FLAG, ha_name, year)

by_month_ha <- bcehs2 %>% filter(ha_name != "Unknown" & phn_flag_update=="Y")%>% #filter(year >= "2018") %>%
   group_by(dateofevent_floor, FN_FLAG, ha_name,year) %>%
  summarise(total_od = n()) %>%
    mutate(percent=round((total_od/sum(total_od))*100, digits=1),
           mm_total= sum(total_od))

overdose_rate_ha <- by_month_ha  %>% 
  dplyr::left_join(pop_combo, by =c("year", "FN_FLAG", "ha_name"))  %>%  
  dplyr::mutate(od_rate=round(100000*total_od/Population, digits=1))
...
export_crude_rates <- overdoselimits %>% 
  select(c(ha_id, ha_name, year, FN_FLAG, fn_flag_fac, dateofevent_floor, total_od, Population, od_rate, LL, UL, conf.level))

flag_dates <- seq(min(export_crude_rates$dateofevent_floor), max(export_crude_rates$dateofevent_floor), by = '2 month')
export_crude_rates <- export_crude_rates %>% 
...
export_crude_rates %>% 
#  filter(FN_FLAG ==1 ) %>% 
  ggplot( aes(x=dateofevent_floor, y=od_rate, group=fn_flag_fac, fill= fn_flag_fac)) + 
...
bc_fn_pop_twone <- bc_fn_pop %>% 
  filter(year ==2019) %>% 
  mutate(year =2021)


bc_fn_pop_t <- bind_rows(bc_fn_pop, bc_fn_pop_tw, bc_fn_pop_twone)
...
od_event_sex <- od_event_sex %>% 
  mutate(year = year(dateofevent_floor))
...
ggplot(sex_specific_rates_limits, aes(x=dateofevent_floor, y=od_rate, group=PAT_GENDER, fill=PAT_GENDER)) + 
...
band_lookup <- read_csv(file = "\\\\fnha.local/groups/Health Surveillance/Health Surveillance - General/lookup_files/r_readin_final_lookup_geography.csv")
lha18 <- band_lookup %>% 
